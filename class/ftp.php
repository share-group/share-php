...
class ftp{
	...
	public static function mkdir($directory, $mode=null){
		...
		if(!self::mkdir(dirname($directory), $mode)){
			return false;
		}
		if(!ftp_mkdir(self::$resource, $directory)){
			return false;
		}
		if(!is_null($mode)){
			self::chmod($mode, $directory);
		}
		return true;
	}
	...
	public static function upload($local_file, $remote_file, $mode=null, $permissions = null){
		if(!file_exists($local_file)){
			throw_exception('ftp_no_source_file:'.$local_file);
		}
		$mode = is_null($mode) ? self::_getTransferMode($local_file) : $mode;
		if(!self::put($remote_file, $local_file, $mode)){
			throw_exception("ftp_unable_to_upload:local_file[{$local_file}]/remote_file[{$remote_file}]");
		}
		if(!is_null($permissions)){
			self::chmod($permissions, $remote_file);
		}
		return true;
	}
	...
	public static function uploadDir($local_dir, $remote_dir, $permissions = null){
		...
		if(count($list) > 0){
			...
			foreach ($list as $k=>$v){
				...
				if(Folder::isDir($local_path)){
					self::uploadDir($local_path, $remote_path);
				} else {
					self::upload($local_path, $remote_path, null, $permissions);
				}
			}
			return true;
		}
	...
	public static function download($remote_file, $local_file, $mode = null){
		...
		if(!self::get($local_file, $remote_file, $mode)) {
			throw_exception("ftp_unable_to_download:local_file[{$local_file}]-remote_file[{$remote_file}]");
		}
		return true;
	}
